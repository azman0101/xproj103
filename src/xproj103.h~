...
data_struct** ip_get(data_struct **ip_array)
{
    struct ifaddrs *ifaddr, *ifa;
    int family, s,i;
    char host[NI_MAXHOST];
    if (ip_array == NULL)
      ip_array = malloc(sizeof(data_struct));
    ...
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        if (family == AF_INET || family == AF_INET6) {
            ...
            printf("\taddress: <%s>\n", host);
            ip_array[i] = calloc(1, sizeof(data_struct));
	    ...
	    // strncpy((char*)ip_array->name,ifa->ifa_name,sizeof(ifa->ifa_name));
