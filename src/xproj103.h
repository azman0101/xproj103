...
pthread_mutex_t ifcpu_mutex = PTHREAD_MUTEX_INITIALIZER;
...
bool parser_xdr(void* buffer_xdr)
{
      // Creation d'un "parser" XDR afin de reconstruire les données reçues dans buff
	   ...
	   xdr_cpu(&xdrs, ifcpu_rcv_array[i]->cpu);
	    
	     
	   printf("CPU: %lu\n", (long unsigned)ifcpu_rcv_array[i]->cpu->cpu_use);
	   printf("FREE MEM: %lu\n", ifcpu_rcv_array[i]->cpu->free_mem);
	   printf("FREE TOT: %lu\n", ifcpu_rcv_array[i]->cpu->total_mem);
	     
	     	
	     while (xdr_if(&xdrs, ifcpu_rcv_array[i]->interface[j]))
	     { 
	       if (!isIpAddress(if_rcv_array[j]->ip)) break;
	       printf("Interface: %s\n", if_rcv_array[j]->name);
	       printf("IP: <%s>\n", if_rcv_array[j]->ip);
	       
	      // syslog_ip_cpu(cpu_rcv_array[i], if_rcv_array[j]);
	       j++;
