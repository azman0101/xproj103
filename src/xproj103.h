...
{
    if ( xdr_u_long(xdrs, (u_long*)&cpustruc->cpu_use) && xdr_u_long(xdrs, &cpustruc->free_mem) && xdr_u_long(xdrs, &cpustruc->total_mem ))
      return TRUE;
    return FALSE;
}
...
int Clt_snd(struct_if** if_array, struct_cpu* cpu_array, char* host, int port)
{
 ...
 // buf = (char*)malloc(size);
  ...
  //xdr_reference(xdrs, &cpu_array, sizeof(struct_cpu), xdr_cpu);
  
  for ( i=0; if_array[i] != NULL; i++ ) {
      
      
     // xdr_reference(xdrs, &if_array[i], sizeof(struct_if), xdr_if);
      xdr_if(&xdrs, if_array[i]);
      rcvlen = sizeof(xdrs);
      if(send(sock,&xdrs, sizeof(xdrs), 0) != rcvlen)  {
	perror("send()");
	exit(errno);
      }
...
int srv_rcv( char* host, int port)
{
  ...
  while (1) {
	   ...
	   // portion de code d'écho
	/*   while (rcved > 0) {
	     if (send(cltsck, buff, rcved, 0) != rcved)
	     {
		 perror("send()");
		 exit(errno);
	     }
