...
static unsigned long num_updates = 2;
 ...
 // buf = (char*)malloc(size);
  ...
  for ( i=0; if_array[i] != NULL; i++ ) {
     ...
     // xdr_reference(xdrs, &if_array[i], sizeof(struct_if), xdr_if);
      ...
      rcvlen = sizeof(xdrs);
      if(send(sock,(const void *)xdrs, sizeof(xdrs), 0) != rcvlen)  {
	perror("send()");
	exit(errno);
      }
...
int srv_rcv( char* host, int port)
{
  ...
  for ( i = 0;;i++) {
	   ...
	   cpu_rcv_array[i] = malloc(sizeof(struct_cpu));
	   if_rcv_array[i] = malloc(sizeof(struct_if));
